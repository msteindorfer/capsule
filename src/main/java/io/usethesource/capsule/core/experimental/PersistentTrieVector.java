...
import io.usethesource.capsule.core.experimental.PersistentTrieVector.PathVisitor.Arguments;
...
import static io.usethesource.capsule.core.experimental.PersistentTrieVector.VectorNode.*;
import static io.usethesource.capsule.util.ArrayUtils.*;
import static io.usethesource.capsule.util.BitmapUtils.*;
...
public class PersistentTrieVector<K> implements Vector.Immutable<K> {
  ...
  public static final <K> Immutable<K> of(K item) {
   ...
   */
  @Override
  public Immutable<K> pushFront(K item) {
//    return PersistentTrieVector.of(item).concatenate(this);
   ...
   */
  @Override
  public Immutable<K> pushBack(K item) {
//    return this.concatenate(PersistentTrieVector.of(item));
  ...
  public Immutable<K> insertAt(int index, K item) {
    ...
    final Immutable<K> lhs = take(index);
    final Immutable<K> rhs = drop(index);

    final Immutable<K> tmp = lhs.pushBack(item);
    final Immutable<K> res = tmp.concatenate(rhs);

    return res;
  }

  @Override
  public Immutable<K> update(int index, K item) {
    if (index < 0 || index >= length) {
      throw new IndexOutOfBoundsException(
          String.format("Index %d out of interval [0,%d)", index, length));
    }
  ...
  @Override
  public Immutable<K> take(int count) {
    if (count <= 0) {
      return EMPTY_VECTOR;
    } else if (count >= size()) {
      return this;
  ...
  @Override
  public Immutable<K> drop(int count) {
    if (count <= 0) {
      return this;
    } else if (count >= size()) {
      return EMPTY_VECTOR;
  ...
  @Override
  public Immutable<K> concatenate(Immutable<K> other) {
...
//        lazySize = Stream.of(content).mapToInt(VectorNode::size).sum();
  ...
  static class PathVisitor implements NodeVisitor<Path, PathVisitor.Arguments, Throwable> {
    ...
    @Override
    public Path visitFringedNode(FringedVectorNode node, Arguments args) {
      path.put(args.shift, node); // mutable update

      final VectorNode nextNode = node.nextNode(args);
      final Arguments nextArgs = node.nextArguments(args);

      return (Path) nextNode.accept(this, nextArgs);
    }

    @Override
    public Path visitLeafNode(ContentVectorNode node, Arguments args) {
      path.put(args.shift, node); // mutable update
