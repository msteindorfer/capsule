...
export PARAM_VALUE_FACTORY="VF_CAPSULE,VF_SCALA,VF_CLOJURE,VF_PAGURO"
...
if [[ -z "$benchmark_selector" || "$benchmark_selector" == "time" ]]; then
  java -jar $JMH_JAR \
    "JmhListBenchmarks.time(PushBack$|PushFront$|Concatenate$)$" \
    -f 1 -i 5 -wi 5 \
    -p producer=PURE_INTEGER \
    -p size=$PARAM_SIZE \
    -p valueFactoryFactory=$PARAM_VALUE_FACTORY \
    -rf text -rff log-jmh-persistent-vector-time.txt
fi
...
if [[ -z "$benchmark_selector" || "$benchmark_selector" == "space" ]]; then
  java -javaagent:$MEMORY_MEASURER_AGENT -jar $JMH_JAR \
    "JmhListBenchmarks.footprint$" \
    -f 1 -bm ss \
    -p producer=PURE_INTEGER \
    -p size=$PARAM_SIZE \
    -p valueFactoryFactory=$PARAM_VALUE_FACTORY \
    -prof io.usethesource.capsule.jmh.profiler.MemoryFootprintProfiler \
    -rf text -rff log-jmh-persistent-vector-space.txt
fi
